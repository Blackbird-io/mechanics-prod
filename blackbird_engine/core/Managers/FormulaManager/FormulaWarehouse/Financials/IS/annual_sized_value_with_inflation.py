#PROPRIETARY AND CONFIDENTIAL
#Property of Blackbird Logical Applications, LLC
#Copyright Blackbird Logical Applications, LLC 2015
#NOT TO BE CIRCULATED OR REPRODUCED WITHOUT PRIOR WRITTEN APPROVAL OF ILYA PODOLYAKO

#Blackbird Engine
#Module: FW.Financials.IS.FixedValue
"""

Content module for a formula that sets the line to a fixed value from data.

====================  ==========================================================
Attribute             Description
====================  ==========================================================

DATA:
date_created          str in "YYYY-MM-DD"l; date when author wrote module
formula_content       bool; always True for content modules, else catalog skips
formula_author        str; name of person who wrote the module
name                  str; name of formula defined by module

FUNCTIONS:
func()                modifies line and /or business unit on behalf of driver

CLASSES:
n/a
====================  ==========================================================
"""




#imports
#n/a




#globals
formula_content = True
name = "inflation-adjusted annualized value multiplied by unit size."

#FORMULA NAMES SHOULD DESCRIBE THEIR ACTION IN SIMPLE LANGUAGE
#simple names make it easier to reuse functions and avoid errors when creating
#drivers

formula_author = "Ilya Podolyako"
date_created = "2015-06-14"
required_data = ["fixed_monthly_value",
                 "annual_inflation",
                 "ref_year"]

def func(line, business_unit, data, driver_signature):
    """

    func(line, business_unit, parameters, driver_signature) -> None

    Function sets line to an annualized version of the specified monthly
    expense. The annualized expense is a product of the monthly number, unit
    size, and compounded inflation since ref_year. Function then applies
    descriptive tags and any new tags optionally included in data. 

    Data must include:

    -- "annual_inflation"     |   annual inflation, expressed in decimals
    -- "fixed_monthly_value"  |   line value, pre-adjustment base
    -- "ref_year"             |   year when base value was measured

    Data can include:

    -- "new_optional_tags"    |   list of tags to add to line after value
    """
    #
    calc_year = business_unit.life.ref_date.year
    years_from_base = calc_year - data["ref_year"]
    years_from_base = max(0, years_from_base)
    inflation_multiplier = (1 + data["annual_inflation"])**years_from_base
    #
    value = (data["fixed_monthly_value"] *
             business_unit.size *
             inflation_multiplier *
             12)    
    line.setValue(value, driver_signature)
    #
    line.tag("inflation adjusted")
    im_tag = "inflation multiplier: %s" % round(inflation_multiplier,4)
    line.tag(im_tag)
    line.tag("tracks size")
    line.tag("compounds annually")
    line.tag("annualized from monthly base")
    #
    new_optional_tags = data.get("new_optional_tags")
    if new_optional_tags:
        line.tag(*new_optional_tags, field = "opt")
    #
    #always return None
    return None
