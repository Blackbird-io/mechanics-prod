...
#globals
topic_content = True
name = "saas-oriented maintenance revenue per subscriber"
topic_author = "Ilya Podolyako"
date_created = "2015-06-12"
extra_prep = False

#store tags this topic uses multiple times in variables to avoid typos
tg_constant_life_revenue = "constant revenue over lifecycle" #---------------------------------CLEAN TAGS
...
question_names = ["ancillary monthly revenue per subscriber?"]
...
def apply_data(topic, datapoint):
    ...
    c
    """
    #add driver that only writes a per-day service charge during "installation"
    #can ask about per-day per-client charge, or staffing per client ?
