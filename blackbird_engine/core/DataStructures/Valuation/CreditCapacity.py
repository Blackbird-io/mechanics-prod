...
from .ABL import ABL
from .CR_Landscape import CR_Landscape
...
class CreditCapacity(Pattern):
    ...
    def __init__(self,name = "CreditCapacity"):
        Pattern.__init__(self,name)
        abl = ABL()
        landscape = CR_Landscape(standard = {})
        ll = LL()
        self.addElement("abl",abl)
        self.addElement("landscape",landscape)
        self.addElement("ll",ll)
        self.updateLandscape = True
    ...
    def buildSizeLandscape(self, yCurve, scalingFactor,
        ...
        for i in range(len(steps)):
            ...
            print("ref = CR_Reference(mod_scenario)")
            print("ref: ", ref)
            print("ref[bad]['size']: ", ref[bad]["size"])
            print("ref[mid]['size']: ", ref[mid]["size"])
            print("ref[good]['size']: ", ref[good]["size"])
    ...
    def buildSecondaryLandscape(self,s_scape,ref_key = "price",
        ...
        reference that contains every scenario with a given price (8%). There
        may be many such scenarios, so the reference should key them by whatever
        determines their quality. When price is constant, size is the most
        significant variable, so in the p_scape, each reference would contain
        keys from low to high size, pointing to scenarios that show such size
        at the given price (8%).
